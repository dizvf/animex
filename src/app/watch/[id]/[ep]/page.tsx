...
import { getEpisodeSources } from "@/lib/anify";
...
export default async function WatchPage({ params }: WatchPageProps) {
  ...
  // Fetch anime info + sources + skip times in parallel
  const [animeResult, sourcesResult, skipResult] = await Promise.allSettled([
    getAnimeById(id),
    getEpisodeSources(id, ep),
    getAnimeById(id).then((d) => {
      const mal = (d.Media as AniListAnime).idMal;
      if (!mal) return { found: false, results: {}, statusCode: 404 } as AniSkipResult;
      return getSkipTimes(mal, ep);
    }),
  ]);
  ...
  const skipTimes: AniSkipResult =
    skipResult.status === "fulfilled"
      ? skipResult.value
